...
from components.intake_animation import (
    render_intake_progress,
    render_upload_another_link,
...
def init_intake_state():
    ...
    defaults = {
        ...
        "intake_error": None,
        "intake_manual_mode": {},  # Track which fields use manual entry
    }
...
def api_get_all_contracts() -> List[Dict]:
    ...
    try:
        ...
        if isinstance(data, list):
            return [c for c in data if c and isinstance(c, dict)]
...
def render_file_uploader():
    ...
    if uploaded_file and not st.session_state.get("intake_file_id"):
        with st.spinner("Uploading..."):
            ...
            st.session_state["intake_file_id"] = result["file_id"]
            st.session_state["intake_file_path"] = result["file_path"]
            st.session_state["intake_filename"] = result["filename"]
...
def render_question_panel():
    ...
    manual_mode = st.session_state.get("intake_manual_mode", {})
    ...
    if current_q == "title":
        ...
        confidence = extraction.get("title_confidence", 0)
        
        st.markdown("**📝 Contract Title**")
        if confidence >= 0.7 and prefill:
            st.caption(f"Suggested: {prefill[:50]}... ({confidence:.0%})")
    ...
    # ===================== CONTRACT TYPE =====================
        ...
        confidence = extraction.get("contract_type_confidence", 0)
        
        st.markdown("**📋 Contract Type**")
        if confidence >= 0.7 and prefill:
            st.caption(f"Suggested: {prefill} ({confidence:.0%})")
        
        # Always show dropdown + manual option
    ...
    # ===================== PURPOSE =====================
        ...
        confidence = extraction.get("purpose_confidence", 0)
    ...
    # ===================== PARTIES =====================
        ...
        confidence = extraction.get("parties_confidence", 0)
        hint = extraction.get("orientation_hint", "")
        ...
        if use_manual or len(parties) != 2:
            # Manual entry
            our_party = st.text_input("Our Party", key="q_our_party_manual", 
                                       value=parties[0] if len(parties) > 0 else "")
            counterparty = st.text_input("Counterparty", key="q_counterparty_manual",
    ...
    # ===================== RELATED =====================
        ...
        all_contracts = st.session_state.get("intake_all_contracts", [])
        ...
        if all_contracts:
            ...
            for c in all_contracts:
                ...
                party = (c.get("counterparty") or "")[:20]
                options.append(f"{title} ({ctype}) - {party}")
            
            # Only show dropdown if we have valid options beyond "None"
            if len(options) > 1:
                selection = st.selectbox("Select related contract", options, key="q_related")
                
                related_id = None
                if selection != options[0] and len(options) > 1:
                    try:
                        idx = options.index(selection) - 1
                        if 0 <= idx < len(all_contracts):
                            related_id = all_contracts[idx].get("id")
                    except (ValueError, IndexError):
                        related_id = None
            else:
                st.caption("No valid contracts found")
                related_id = None
        else:
            st.caption("No existing contracts found")
            related_id = None
...
def render_complete_summary():
    ...
    st.caption(f"Uploaded: {st.session_state.get('intake_filename', 'Unknown')}")
...
def render_confirmed_panel():
    ...
    # Show more details when complete
    if complete:
        st.markdown("---")
...
def render_progress_panel():
    ...
    # Upload another link when complete
    if complete:
    ...
    if not st.session_state.get("intake_file_id"):
        ...
        st.markdown(f'''
            <div style="background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 8px; padding: 8px 12px; margin-bottom: 16px; display: flex; align-items: center; gap: 8px;">
                ...
                <span style="color: #E2E8F0; font-size: 0.85rem;">{st.session_state["intake_filename"]}</span>
            </div>
