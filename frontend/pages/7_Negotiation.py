...
def update_workflow_stage(contract_id: int, stage: int):
    """Update contract workflow stage via API."""
    try:
        resp = requests.post(
            f"{API_BASE}/contracts/{contract_id}/stage",
...
if 'neg_stage_updated' not in st.session_state:
    st.session_state.neg_stage_updated = set()  # Track contracts promoted to stage 3
...
def z1_contract_and_filters():
    ...
    if contracts is not None and len(contracts) > 0:
        ...
        # Auto-update workflow stage to 3 (Negotiation) when entering this page
        if selected_id and selected_id not in st.session_state.neg_stage_updated:
            if update_workflow_stage(selected_id, 3):
                st.session_state.neg_stage_updated.add(selected_id)
    else:
